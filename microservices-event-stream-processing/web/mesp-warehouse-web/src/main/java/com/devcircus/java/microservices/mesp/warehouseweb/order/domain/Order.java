...
import com.devcircus.java.microservices.mesp.warehouseweb.order.event.OrderEvent;
...
public class Order extends Aggregate<OrderEvent, Long> {
    ...
    @Override
    public Link getId() {
        return new Link(new UriTemplate("http://order-web/v1/orders/{id}").with("id", TemplateVariable.VariableType.PATH_VARIABLE)
                .expand(getIdentity())
